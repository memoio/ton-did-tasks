...
export function Footer ( props ) {
    return (
        <section className="px-8 w-full fixed bottom-0 pb-8 bg-white dark:bg-transparent pt-2">
            <footer className="bg-white w-full mx-auto grid grid-cols-6 py-2 inset-x-4 z-30 gap-4 items-end dark:bg-white/5 dark:border dark:border-solid dark:border-white/10 dark:pb-2 dark:rounded-lg dark:bottom-6 backdrop-blur-xs">
                ...
                { props.active == 'blindboxv2'? <FooterKey title={"BlindBox"} image={"/blindbox-active.svg"} active={true} />: <FooterKey title={"BlindBoxV2"} image={"/blindbox.svg"} url={"/blindboxv2"} active={false} /> }
            </footer>
        </section>
    )
}
